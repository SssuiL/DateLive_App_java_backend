...
- 已实现 163 个 HTTP 入口（其中超级喜欢为已取消的 410 兼容入口）和 2 个 WebSocket 入口：账号、资料、通知、图片、后台审核、注销/恢复、用户搜索、好友和拉黑，以及聊天会话、文本/图片/语音/视频/文件消息和静态贴纸/动态 GIF、引用回复、已读，以及撤回/搜索/个人隐藏和清空。到期清理已实现并在隔离库测试，自动 Worker 已获授权开启，每分钟清理超过 45 天冷静期的注销账号；可用 JAVA_ACCOUNT_ERASURE_ENABLED=false 关闭（本地启动脚本默认开启，使用 -DisableAccountErasure 可关闭）；短信仍为开发模拟。
...
完整回归 **374 项通过**；最终协议版本配置另经 16 项专项测试验证并打包，本地 11 项 HTTP 验收通过。迁移申请、人工审核、暂停恢复、脱敏、官方通知及注销清理。默认协议版本沿用旧版 2026-07，开播资格检查默认启用；真实身份供应商与直播链路仍待后续验收。详见 [迁移说明](docs/主播资格迁移说明.md) 和 [本机报告](docs/local-host-verification-smoke.json)。

## 直播房间与开播检查（V24）

累计验证 **393 项不同测试**：完整回归 392 项通过，最终内容类型修复及新增回归经 19 项直播测试验证、打包；本地 14 项 HTTP 验收通过。支持模拟房间生命周期、一次性开播票据、参与者进出及受控封面。显式 -DevelopmentLive 开启本地模拟，默认及生产禁用。详见 [迁移说明](docs/直播房间与预检迁移说明.md) 和 [本机报告](docs/local-live-room-smoke.json)。直播广播、治理、送礼、预约及真实推流继续迁移。
