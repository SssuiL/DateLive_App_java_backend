...
$items=@(foreach($route in $legacy.routes){
 ...
 [ordered]@{method=$route.method;legacy_path=$route.path;source=$route.source;java_path=$(if($match){$match.path}else{$null});status=$(if($match.status -eq 'retired_by_product_decision'){'retired_by_product_decision'}elseif($match){'route_mapped_contract_audit_required'}else{'not_mapped'});note=$(if($match.status -eq 'retired_by_product_decision'){'User removed super-like on 2026-09-17; compatibility endpoint returns 410 without side effects.'}else{'Route presence alone is not functional parity. Compare schemas, permissions, state transitions, events and failure cases before closing.'})}
})
$report=[ordered]@{legacy_commit=$legacy.summary.legacy_commit;legacy_http_routes=$legacy.summary.http_routes;legacy_websocket_routes=$legacy.summary.websocket_routes;java_verified_tests=$current.verified_tests;mapped_routes=@($items|Where-Object status -eq 'route_mapped_contract_audit_required').Count;retired_routes=@($items|Where-Object status -eq 'retired_by_product_decision').Count;unmapped_routes=@($items|Where-Object status -eq 'not_mapped').Count;scope='Static route cross-reference; excludes Java-only additions and does not claim full semantic or supplier parity.';routes=$items}
