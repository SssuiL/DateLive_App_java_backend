...
try{
 ...
 & (Join-Path $PSScriptRoot 'start-local.ps1') -DevelopmentSms -DevelopmentAdmin
 if($LASTEXITCODE -ne 0){throw 'Restart with enabled media worker failed'}
 $workerPaused=$false
 ...
 $report=@{checked_at=[DateTime]::UtcNow.ToString('o');target=$javaBase;schema='V16';result='passed'
  checks=@('queued_without_download','owner_only_status','cancel_queued_task','actual_api_restart_recovery','image_and_video_processed','malformed_input_failed','review_and_message_send','thumbnail_decode','range_206','hide_revokes_access','bundle_deletion')
 ...
 [IO.File]::WriteAllText((Join-Path $javaProjectRoot 'docs/local-media-jobs-smoke.json'),($report|ConvertTo-Json -Depth 5),[Text.UTF8Encoding]::new($false))
